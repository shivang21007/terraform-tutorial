...
sudo sed -i "s|.*<h1>.*|<h1>Welcome to nginx! $(hostname)</h1>|" /var/www/html/index.html
# If the file above doesn't exist (e.g. on Ubuntu), try the debian specific one:
sudo sed -i "s|.*<h1>.*|<h1>Welcome to nginx! $(hostname)</h1>|" /var/www/html/index.nginx-debian.html
